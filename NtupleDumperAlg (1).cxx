  ...
  for (const Trk::Track* track : *trackCollection)
  {
    ...
    for (auto measurement : *(track->measurementsOnTrack())) {
        ...
        if (cluster != nullptr) {
            ...
            //std::cout << count++ << " " << station << " " << stationMap.size() << std::endl;
            layerMap.emplace(station, layer);
        }
    }
    std::cout << stationMap.count(0) + stationMap.count(1)+stationMap.count(2)+stationMap.count(3) << std::endl;
    ...
    m_nHit0.push_back(std::count_if(stationVector.begin(), stationVector.end(), [](int p){return p==0;}));
    m_nHit1.push_back(std::count_if(stationVector.begin(), stationVector.end(), [](int p){return p==1;}));
    m_nHit2.push_back(std::count_if(stationVector.begin(), stationVector.end(), [](int p){return p==2;}));
    m_nHit3.push_back(std::count_if(stationVector.begin(), stationVector.end(), [](int p){return p==3;}));
    n_clu.push_back(stationVector.size());
